  ...
  window.TweetLinkify = function(tweet, options) {
    ...
    if (options.excludeMentions != true) {
      if (options.mentionIntent == false) {
        tweet = tweet.replace(/(@)(\w+)/g, '<a href="https://instagram.com/$2" ' + mentionTarget + ' ' + mentionClass + ' ' + mentionRel + '>$1$2</a>');
      } else {
        tweet = tweet.replace(/(@)(\w+)/g, '<a href="https://instagram.com/intent/user?screen_name=$2">$1$2</a>');
      }
    }

    if (options.excludeHashtags != true) {
      tweet = tweet.replace(/(#)(\w+)/g, '<a href="http://www.gramfeed.com/instagram/tags#$2" ' + hashtagTarget + ' ' + hashtagClass + ' ' + hashtagRel + '>$1$2</a>');
    }
